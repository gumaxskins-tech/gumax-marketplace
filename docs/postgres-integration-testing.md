...
The test constructs a real `PrismaClient` only in the test harness, injects it into `PrismaCaptureTransactionManager`, and verifies the claim → append → complete → Hold capture flow. It also includes a deliberate post-write `ROLLBACK_PROBE`: the Hold must remain `ACTIVE`, and no LedgerEntry or Ledger idempotency record may persist; the same key must then be claimable in a new aborted transaction. The commit and rollback scenarios have passed on the VPS. A third scenario releases two real transactions at a test-only barrier before `claim`: one same-key result must be `COMPLETED` with exactly one LedgerEntry and IdempotencyRecord, while two different keys produce two independent results. It uses no `DATABASE_URL` fallback. VPS execution of the concurrency scenario remains pending. The first PostgreSQL execution reached cleanup, where the database correctly rejected deletion of a Ledger idempotency record with `P0001` (`Ledger idempotency records cannot be deleted`). The harness now uses UUID-scoped fixtures and intentionally retains all resulting financial artifacts; resetting or recreating the disposable database belongs to the environment lifecycle, never test cleanup.
