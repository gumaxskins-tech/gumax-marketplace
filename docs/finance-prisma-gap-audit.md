...
| `LedgerIdempotencyStore` | `claim`, `complete` / `LedgerIdempotencyClaim`, `LedgerEntry` | `IdempotencyRecord`, `LedgerEntry` | `@@unique([scope, key])`, typed nullable-during-transaction `ledgerEntryId` FK, unique result reference, deferred Ledger lifecycle triggers, and runtime claim/complete contract are prepared | Prisma persistent store and transaction manager are not integrated; credit/debit remain outside a production transaction boundary | **SCHEMA_AND_RUNTIME_CONTRACT_READY_PERSISTENT_ADAPTER_PENDING** |
