...
| `LedgerIdempotencyStore` | `get`, `set` / `LedgerEntry` | `IdempotencyRecord`, `LedgerEntry` | `@@unique([scope, key])` prevents duplicate keys | **Blocked for a safe adapter:** `entityId` is nullable, untyped, and has no `LedgerEntry` FK; the schema does not enforce a non-null ledger result for a completed `LEDGER` record. The current `get`/append/`set` port flow also has no atomic claim primitive before append, so `find`/append/record would race even with the existing unique constraint | Requires a typed, non-null ledger result reference plus claim/complete in the same ledger transaction. **PARTIAL — SCHEMA/TRANSACTION BLOCKED** |
