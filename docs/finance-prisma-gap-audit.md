...
| `HoldRepository` | `getById`, `save`, `listActiveByWallet`, `count` / `Hold` | `WalletHold` | Wallet FK; `Decimal(20,8)` amount/currency; `createdAt`, nullable `expiresAt`, idempotency key; `ReservationStatus.CAPTURED`; `@@index([walletId, status])` | Focused Hold schema migration is prepared. `PrismaHoldRepository`, PostgreSQL migration execution/integration testing, and the production capture transaction manager remain pending | Hold update and ledger append require one transaction. **SCHEMA_PREREQUISITES_PREPARED_ADAPTER_PENDING** |
