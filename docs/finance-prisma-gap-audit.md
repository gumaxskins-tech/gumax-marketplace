...
| `HoldRepository` | `getById`, `save`, `listActiveByWallet`, `count` / `Hold` | `WalletHold` | Wallet FK; `Decimal(20,8)` amount/currency; `createdAt`, nullable `expiresAt`, idempotency key; `ReservationStatus.CAPTURED`; `@@index([walletId, status])` | `PrismaHoldRepository` is implemented with a transaction-client-compatible `walletHold` delegate and lossless ACTIVE/RELEASED/CAPTURED conversion. `createPrismaCaptureTransactionResources(client)` composes it with the Ledger repository and Ledger idempotency store from the same client; the narrow `CaptureTransactionContext` now matches that boundary without requiring a false full finance context. PostgreSQL migration execution/integration testing and the production capture transaction manager remain pending | Hold update and ledger append require one transaction. **IMPLEMENTED_NOT_POSTGRES_INTEGRATION_TESTED** |
