...
| `LedgerIdempotencyStore` | `get`, `set` / `LedgerEntry` | `IdempotencyRecord`, `LedgerEntry` | `@@unique([scope, key])`, typed nullable-during-transaction `ledgerEntryId` FK, unique result reference, and deferred Ledger lifecycle triggers are prepared in migration | Runtime still uses `get`/`set`; the approved claim/complete contract, persistent store, and transaction manager are not integrated | **RESOLVED_IN_MIGRATION_NOT_RUNTIME_INTEGRATED** |
