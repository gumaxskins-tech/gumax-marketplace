...
**Progress note (P0-A1.1d-6):** the six idempotency/deduplication ports, `LedgerRepository`, `WalletRepository`, `HoldRepository`, `OrderRepository`, and their in-memory adapters have since been converted to async. FinanceService awaits the affected store, ledger, wallet, hold, and order calls; wallet creation, `balance()`, `available()`, ledger posting, credit, debit, hold, release, capture, order creation, order reads, cancellation, and webhook order transitions now return promises. Payment, refund, and inventory-reservation repositories remain synchronous. `capture()` is still not atomic between its hold transition and ledger append; order/reservation and webhook payment/order/event-dedup/audit remain non-atomic. The signature tables and propagation counts below describe the audited pre-conversion baseline.
