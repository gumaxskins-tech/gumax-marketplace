...
**Progress note (P0-A1.1d-5):** the six idempotency/deduplication ports, `LedgerRepository`, `WalletRepository`, `HoldRepository`, and their in-memory adapters have since been converted to async. FinanceService awaits the affected store, ledger, wallet, and hold calls; wallet creation, `balance()`, `available()`, ledger posting, credit, debit, hold, release, capture, and `getEntries()` now return promises. Order, payment, refund, and inventory-reservation repositories remain synchronous. `capture()` is still not atomic between its hold transition and ledger append. The signature tables and propagation counts below describe the audited pre-conversion baseline.
