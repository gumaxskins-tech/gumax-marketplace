...
**Progress note (P0-A1.2d):** P0-A1.1d is complete: the six idempotency/deduplication ports, all seven main repositories, and their in-memory adapters have been converted to async. FinanceService awaits every affected store and repository call, including its separate item-only `reserve()` operation. `capture()` runs through FinanceTransactionManager and uses `tx.holdRepository`, `tx.ledgerRepository`, and `tx.ledgerIdempotencyStore` for its boundary. `createOrder()` also runs through the manager and uses `tx.orderIdempotencyStore`, `tx.inventoryReservationRepository`, and `tx.orderRepository`. For `OrderItem.source === "INVENTORY"`, the required `inventoryItemId` is reserved before the Order is saved; `source === "ORDER"` does not reserve inventory. Verified `webhook()` processing runs through the manager and uses `tx.paymentEventDedupStore`, `tx.paymentRepository`, and `tx.orderRepository`; provider verification remains outside the boundary. `refund()` runs through the manager and uses `tx.refundIdempotencyStore`, `tx.paymentRepository`, and `tx.refundRepository`. Audit remains the service's in-memory collection outside the transaction context. P0-A1.2 is complete as an in-memory transaction-composition phase only. This is composition only: InMemoryFinanceTransactionManager has no rollback or production atomicity. Reservation/order, webhook payment/order/event-dedup/audit, and refund idempotency/refund/audit remain non-atomic. In-memory reservation exclusivity applies only to a shared adapter in one process; it is neither distributed nor crash-safe. Refunds retain record-only semantics with no ledger or settlement effect. The signature tables and propagation counts below describe the audited pre-conversion baseline.
