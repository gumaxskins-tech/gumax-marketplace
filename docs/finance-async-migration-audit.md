...
**Progress note (P0-A1.1d-4):** the six idempotency/deduplication ports, `LedgerRepository`, `WalletRepository`, and their in-memory adapters have since been converted to async. FinanceService awaits the affected store, ledger, and wallet calls; wallet creation, `balance()`, `available()`, ledger posting, credit, debit, hold, capture, and `getEntries()` now return promises. Hold, order, payment, refund, and inventory-reservation repositories remain synchronous. The signature tables and propagation counts below describe the audited pre-conversion baseline.
