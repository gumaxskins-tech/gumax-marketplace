...
**Progress note (P0-A1.1d-3):** the six idempotency/deduplication ports, `LedgerRepository`, and their in-memory adapters have since been converted to async. FinanceService awaits the affected store and ledger calls; `balance()`, `available()`, ledger posting, credit, debit, hold, capture, and `getEntries()` now return promises. Wallet, hold, order, payment, refund, and inventory-reservation repositories remain synchronous. The signature tables and propagation counts below describe the audited pre-conversion baseline.
