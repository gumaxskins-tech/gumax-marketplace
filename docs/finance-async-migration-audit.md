...
**Progress note (P0-A1.1d-9):** the six idempotency/deduplication ports, all seven main repositories, and their in-memory adapters have since been converted to async. FinanceService awaits every affected store and repository call, including its separate item-only `reserve()` operation. `createOrder()` does not currently call the reservation repository, so this conversion does not couple order creation to reservation. `capture()` is still not atomic between its hold transition and ledger append; reservation/order, webhook payment/order/event-dedup/audit, and refund idempotency/refund/audit remain non-atomic. In-memory reservation exclusivity applies only to a shared adapter in one process; it is neither distributed nor crash-safe. Refunds retain record-only semantics with no ledger or settlement effect. The signature tables and propagation counts below describe the audited pre-conversion baseline.
