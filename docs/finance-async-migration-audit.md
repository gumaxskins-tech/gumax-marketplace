...
**Progress note (P0-A1.1d-8):** the six idempotency/deduplication ports, `LedgerRepository`, `WalletRepository`, `HoldRepository`, `OrderRepository`, `PaymentRepository`, `RefundRepository`, and their in-memory adapters have since been converted to async. FinanceService awaits the affected store, ledger, wallet, hold, order, payment, and refund calls; wallet creation, `balance()`, `available()`, ledger posting, credit, debit, hold, release, capture, order creation, order reads, cancellation, payment creation, payment reads, webhook payment/order transitions, and refund payment/refund-record reads and writes now return promises. Inventory-reservation repositories remain synchronous. `capture()` is still not atomic between its hold transition and ledger append; order/reservation, webhook payment/order/event-dedup/audit, and refund idempotency/refund/audit remain non-atomic. Refunds retain record-only semantics with no ledger or settlement effect. The signature tables and propagation counts below describe the audited pre-conversion baseline.
