...
**Progress note (P0-A1.1d-7):** the six idempotency/deduplication ports, `LedgerRepository`, `WalletRepository`, `HoldRepository`, `OrderRepository`, `PaymentRepository`, and their in-memory adapters have since been converted to async. FinanceService awaits the affected store, ledger, wallet, hold, order, and payment calls; wallet creation, `balance()`, `available()`, ledger posting, credit, debit, hold, release, capture, order creation, order reads, cancellation, payment creation, payment reads, webhook payment/order transitions, and refund payment lookup now return promises. Refund and inventory-reservation repositories remain synchronous. `capture()` is still not atomic between its hold transition and ledger append; order/reservation and webhook payment/order/event-dedup/audit remain non-atomic. The signature tables and propagation counts below describe the audited pre-conversion baseline.
